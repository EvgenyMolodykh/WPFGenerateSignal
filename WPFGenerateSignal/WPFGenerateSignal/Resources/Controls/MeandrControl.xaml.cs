...
{
    public partial class MeandrControl : UserControl
    {
        ...
        private void OnViewModelPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
                ...
                e.PropertyName == nameof(SignalViewModel.PointsCount))
            {
                ...
                System.Diagnostics.Debug.WriteLine($"Параметр {e.PropertyName} изменен, но график меандра не перестроен");
            }
        ...
        {
            _plotModel = new PlotModel { Title = "Меандр" };
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    for (int i = 0; i < pointsCount; i++)
                    {
                        ...
                        double angle = TwoPI * frequency * animatedX + phase;
                        double sinValue = Math.Sin(angle);
                        double value = sinValue >= 0 ? amplitude : -amplitude;
                ...
                catch (Exception ex)
                {
                    Application.Current.Dispatcher.Invoke(() =>
                    {
                        System.Diagnostics.Debug.WriteLine($"Ошибка генерации меандра: {ex.Message}");
                    });
                }
            }, cancellationToken);
        }
